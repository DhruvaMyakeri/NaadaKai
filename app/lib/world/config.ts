...
export const NO_CHARACTER_SUFFIX =
  " Strict DISEMBODIED first-person point of view at ALL times: the scene is seen directly through the viewer's own eyes with NO body of any kind ever present. There are NO hands, NO arms, NO fingers, NO feet, NO legs, NO shoulders, NO shadow of a person, NO reflection of a person, NO avatar, NO playable character, NO figure, NO person, NO creature, NO animal, NO robot, NO vehicle ever visible or ever spawned as the viewpoint's own body. The viewpoint is a free, floating, disembodied point in space — never render an object that represents the viewer, never introduce hands into the frame, never let the perspective attach to, ride, mount, drive, possess, inhabit, follow, or take control of any entity, object, figure, or creature. Nothing in the scene is ever a walking, turning, driven, or controllable subject that the perspective is bound to. The view never switches to third person and never embodies or trails anything. Any people already present in the scene are environment only — distant silhouettes, background dancers, crowd texture — never the viewer's own body and never something the viewpoint occupies. The world moves; the viewpoint only observes.";
...
export const STATIC_CAMERA_SUFFIX =
  " The camera is a completely fixed, tripod-locked viewpoint driven 100% by direct user input and NOTHING else: it NEVER pans, tilts, rolls, rotates, orbits, dollies, tracks, zooms, sways, bobs, creeps, drifts, glides, floats forward, pushes in, pulls back, ascends, descends, spins, sweeps, or moves through space on its own. There is NO automatic, idle, ambient, atmospheric, cinematic, or flythrough camera motion of any kind — no lazy drift, no slow float, no gentle sway, no breathing motion, nothing. The viewpoint stays perfectly still unless and until the user's own look/move input directly moves it, and even then it moves ONLY exactly as much as that input dictates and stops the instant input stops. Only the environment itself moves; the viewpoint is a locked observation point. Ignore any instruction, tendency, or default in the underlying model to introduce autonomous camera motion.";
